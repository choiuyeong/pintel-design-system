...
export const LIBRARY_TEMPLATES = {
  ...
  'library-selective-eventdetail': {
    ...
    description: 'PREVAX 4 선별관제 이벤트 상세를 디자인 시스템 토큰으로 재현한 예시로, F-9 "주변 카메라 보기" 기능의 진입 컷(①)입니다. 선별관제 모니터링(운영 중) 화면 위에 이벤트 상세가 별도 팝업(모달 다이얼로그)으로 떠오르는 구조이며(딤 배경 + 중앙 다이얼로그, ✕로 닫으면 모니터링 화면으로 복귀), 상세 패널의 [이벤트 영상 · 주변 카메라 보기 · 실시간 영상] 버튼 줄에서 "주변 카메라 보기" 버튼을 되살렸습니다. 좌측 이벤트 목록에서 선택한 이벤트의 카메라가 그룹(group_camera_info)에 속하면 버튼이 활성화되어 클릭 시 F-9 조사 격자창(투망관제 · 주변 카메라 보기)이 다시 부유 창 팝업으로 열리고, 그룹이 없으면 버튼이 비활성화되며 "이 카메라는 그룹에 속해 있지 않습니다." 안내 툴팁을 노출합니다((a)그룹 있음 / (b)그룹 없음 두 상태를 목록에서 오가며 확인). 다이얼로그(스냅샷·이벤트 헤더·조치내역·이벤트 판정·저장)와 배경 모니터링 화면은 기존 선별관제 화면들과 동일한 팔레트·토큰·위계를 따릅니다. 청색 강조는 정본 토큰(#0066FF/#3385FF/#0052CC)만 사용합니다.',
    uses: ['Top navigation', 'Tab', 'Popup', 'Button', 'Content badge', 'Tooltip', 'Checkbox', 'List cell'],
  },
  'library-unassigned': {
