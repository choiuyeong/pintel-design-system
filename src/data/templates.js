...
export const LIBRARY_TEMPLATES = {
  'library-dashboard': {
    ...
    uses: ['Top navigation', 'Tab', 'Card', 'Table', 'List cell', 'Progress indicator', 'Content badge'],
  },
  'library-signup': {
    title: '회원가입',
    ...
    uses: ['Text field', 'Checkbox', 'Button', 'Text button'],
  },
  'library-login': {
    title: '로그인',
  ...
  'library-selective': {
    ...
    uses: ['Top navigation', 'Tab', 'Select', 'Checkbox', 'Chip', 'Button', 'Card', 'List cell', 'Content badge'],
  },
  'library-selective-active': {
    title: '선별관제 모니터링 (운영 중)',
    description: 'PREVAX 4 선별관제 모니터링의 "운영 중(이벤트 적재)" 상태를 디자인 시스템 토큰으로 재현한 예시입니다. 빈 상태 예시(선별관제 모니터링)와 동일한 타이틀바·탭 크롬, 좌측 검색 조건 패널(320px) 규격, 우측 위험도 3밴드(위험·경고·주의) 구조를 공유하되 실제 관제 부하를 채웁니다. 좌측 "최근 30분 이벤트" 리스트는 썸네일·이벤트명·카메라·시각과 함께 경과 시간·미조치 상태를 병기하고, 각 밴드 헤더는 유형별 누적 카운트 칩(발생 0건 유형은 낮은 위계로 dim 처리해 병기)을, 카드 영역은 정탐/오탐 처리 카드(경과 배지·스냅샷·검지 박스·이벤트명·카메라·시각)를 표시합니다. 미처리 이벤트가 없는 밴드는 빈 상태 안내를 노출합니다. 좌측 최근 이벤트나 중앙 카드를 클릭하면 우측 "상세정보" 패널(고정 사이드바)이 채워집니다 — 스냅샷·이벤트/카메라/객체/알람 등급/일시·그룹과 [이벤트 영상·실시간 영상] 버튼, 조치내역·이벤트 판정(정탐/오탐)·조치자 명·저장(정본 PREVAX 3분할 레이아웃). 원본 관제 화면의 원색 대신 선별관제 패밀리 정본 팔레트(위험 #F0436A·경고 #C9847A·주의 #F5EFE0)와 동일 카드/칩/컨트롤을 사용해 Library 선별관제 화면들과 시각 일관성을 유지합니다.',
    uses: ['Top navigation', 'Tab', 'Select', 'Checkbox', 'Chip', 'Button', 'Card', 'List cell', 'Content badge', 'Text field'],
  },
  'library-selective-eventdetail': {
    title: '선별관제 — 이벤트 상세 · 주변 카메라 보기 (F-9 진입)',
    description: 'PREVAX 4 선별관제 이벤트 상세를 디자인 시스템 토큰으로 재현한 예시로, F-9 "주변 카메라 보기" 기능의 진입 컷(①)입니다. 선별관제 모니터링(운영 중)의 3분할 레이아웃(좌: 검색 조건 + 최근 이벤트 리스트 / 중: 위험도 3밴드 이벤트 카드 / 우: 상세정보 패널)에서, 좌측 최근 이벤트나 중앙 카드를 클릭하면 우측 "상세정보" 패널(팝업이 아닌 고정 패널)이 채워집니다. 상세정보에는 스냅샷·이벤트 명·카메라 명·객체·알람 등급·일시·그룹과 [이벤트 영상 · 실시간 영상] 버튼, 그리고 되살린 "주변 카메라 보기" 버튼, 조치내역·이벤트 판정(정탐/오탐)·조치자 명·저장이 들어갑니다. 선택한 이벤트의 카메라가 그룹(group_camera_info)에 속하면 "주변 카메라 보기"가 활성화되어 클릭 시 F-9 조사 격자창(투망관제 · 주변 카메라 보기)이 부유 창 팝업으로 열리고, 그룹이 없으면 비활성화되며 "이 카메라는 그룹에 속해 있지 않습니다." 안내 툴팁을 노출합니다(CAM 5 = 그룹 미지정 예시). 전 구성은 기존 선별관제 화면들과 동일한 팔레트·토큰·위계를 따르며, 청색 강조는 정본 토큰(#0066FF/#3385FF/#0052CC)만 사용합니다.',
    uses: ['Top navigation', 'Tab', 'Button', 'Content badge', 'Tooltip', 'Checkbox', 'List cell', 'Text field'],
  },
  'library-unassigned': {
    ...
    uses: ['Top navigation', 'Icon button', 'Button', 'Content badge', 'Pagination', 'Fallback view', 'Video grid'],
  },
  'library-selective-away': {
    ...
    uses: ['Top navigation', 'Tab', 'Section message', 'Card', 'Button', 'Content badge', 'Tree/List'],
  },
  'library-live': {
    title: '실시간 영상',
    ...
    uses: ['Top navigation', 'Tab', 'Switch', 'Select', 'Segmented control', 'Pagination', 'Toolbar', 'Tree/List', 'Video grid'],
  },
  'library-dragnet': {
    title: '투망관제 — 주변 카메라 보기 (F-9)',
    description: 'PREVAX 4 투망관제(F-9) "주변 카메라 보기" 조사 격자창을 디자인 시스템 토큰으로 재현한 예시입니다. 이벤트/카메라를 기준으로 "주변 카메라 보기"를 누르면 같은 그룹 카메라들을 격자로 함께 띄우는 별도(비저장) 조사 창으로, F-7 그룹 격자 엔진(LiveMonitorWindow)을 재사용합니다. 별도 창 크롬(타이틀바: PREVAX 4 | 그룹명 · "주변 카메라 보기(조사)")과 좌측 채널목록을 제거한 격자 full width(3×3), 하단 분할(1/4/9/16…)·페이지 바로 구성됩니다. 기준 카메라는 재배치로 1페이지 첫 칸에 오며 청색으로 강조(테두리 T.primaryStrong 2px + "기준" 배지 T.primary)해, FocusPin(고정, cautionary amber)과 의미상 분리합니다. 검지 불가역·읽기전용이며, 창은 이동·리사이즈 가능. 원본 HI-FI의 폐기 Primary 계열 색은 정본 토큰(#0066FF/#3385FF/#0052CC)으로 매핑했습니다.',
    uses: ['Content badge', 'Pagination', 'Segmented control', 'Video grid', 'Icon button'],
  },
  'library-auto-switch': {
    title: '실시간 영상 — 이벤트 자동 전환 설정 (F-6)',
    description: 'PREVAX 4 F-6 "이벤트 검지 트리거 자동 전환" 설정 UI를 디자인 시스템 토큰으로 재현한 예시입니다. 실시간영상 "영상 옵션" 툴바에 "자동 전환" 토글(+▾)을 추가하고, ▾를 누르면 설정 팝오버가 열립니다 — ① 자동 전환 기준 등급(주의·경고·위험 3등급 select, 긴급 제외, 선별관제 목록 IN(2,3,4)과 동일 범위, 기본 위험) ② 추출 정지 시간(초 숫자 입력, 최소 1초, 기본 10) ③ 안내(i: 검지 시 해당 카메라를 별도 창으로 자동 추출·메인 그리드 무전이 / !: D-3 이벤트 자동 팝업과 공존). 헤더에 "사용자별" 배지. 등급 색은 정본 3-tier(주의=positive·경고=cautionary·위험=error)로 색만이 아닌 등급 텍스트 병기, 스위치 ON은 정본 Primary(원본 HI-FI의 Azure/폐기 Primary 대체). 실제 추출은 기존 별도 창(LiveVideoStream) 재사용이며 설정 팝오버에는 미리보기 창을 넣지 않습니다. 대신 표출 동작을 인터랙션으로 시연합니다 — 자동 전환 ON 상태에서 그리드의 검지 카메라(위험 등급) 셀을 클릭하면, 해당 카메라가 별도 창(LiveVideoStream: 카메라명 타이틀 · 16:9 · LIVE · 검지 박스)으로 그리드 위에 자동 추출되고(메인 그리드는 무전이·그대로), ✕로 닫습니다.',
    uses: ['Top navigation', 'Tab', 'Switch', 'Select', 'Number field', 'Toolbar', 'Section message', 'Video grid', 'Popup'],
  },
  'library-auto-window-unified': {
    title: '이벤트 자동 창 통합안 (제안)',
    description: 'PREVAX 4에서 자동으로 뜨는 두 창 — D-3 이벤트 자동 팝업(발생영상)과 F-6 자동 전환(라이브) — 을 하나의 자동 창으로 통합하는 제안(doc3)을 비교 검토용으로 재현한 화면입니다. 현재는 발생영상 팝업과 라이브 추출 창이 각각 떠서 겹치고 서로 안 닮았는데, 통합안은 같은 창에서 발생 시각 클립(발생영상·1회 재생·발생 시점 마커) → 라이브(LIVE)로 이어지며 BBox·이벤트명(침입·위험) 맥락과 창 크롬을 하나로 유지합니다. 상단 제안 배너, 통합 자동 창(발생영상/라이브 단계 토글), 통합 포인트 칩(발생 시각→라이브·박스/이벤트명 맥락·발생 시점 표시·창 하나로 일관)으로 구성됩니다. 색·타이포·간격은 토큰만 사용하며(위험=error·라이브=positive·강조=Primary), 확정 화면이 아니라 의사결정용 제안입니다.',
    uses: ['Top navigation', 'Tab', 'Popup', 'Segmented control', 'Content badge', 'Chip', 'Progress indicator', 'Video grid'],
  },
  'library-live-focus': {
    title: '실시간 영상 — Focus(고정)',
    description: 'PREVAX 4 실시간영상 화면의 F-8 "고정 집중" 변형을 디자인 시스템 토큰으로 재현한 인터랙티브 예시입니다. 실시간영상 크롬(타이틀바·탭·좌 지역 트리·영상 옵션 툴바·영상 그리드·하단 순환 바)을 공유하며, 관제사가 특정 카메라를 "고정(핀)"하면 자동 순환 중에도 그 칸은 유지됩니다. 고정 표시는 cautionary(#FFA938) 핀 배지 + 2px 테두리로 하고, 비고정 셀은 마우스를 올리면 "고정" 칩이 나타나 클릭으로 토글됩니다(정본 상호작용). 모든 칸을 고정하면 순환이 멈추고 격자 상단에 반투명 안내 띠("모든 칸을 고정하여 자동 순환이 멈춰 있습니다")가 뜨며, 고정 0개면 일반 실시간영상과 동일(무변화)합니다. 고정은 화면 표시/레이아웃만 바꾸고 검지·순환 엔진은 건드리지 않습니다. 각 셀 좌상단 이벤트 배지는 어떤 검지가 걸렸는지 알리는 읽기 전용 상태 표시이며(클릭 컨트롤 아님), 카메라별 분석영역(ROI 원근 경계)·검지 박스(positive) 오버레이 표시 여부는 셀 우클릭 메뉴의 "이벤트 ON/OFF" 항목(눈 아이콘 + 상태 라벨 — ON=positive·OFF=muted)으로 켜고 끕니다. 상시 노출되는 셀 위 토글을 두지 않아 영상이 깔끔하고, 전역 "영상 옵션"(이벤트 필터·분석 영역) 토글과 역할이 겹치지 않으며(표시 토글이라 검지 엔진과 무관) 기존 우클릭 메뉴(고정·주변 카메라 보기)와 상호작용이 일관됩니다.',
    uses: ['Top navigation', 'Tab', 'Switch', 'Chip', 'Content badge', 'Section message', 'Segmented control', 'Pagination', 'Toolbar', 'Tree/List', 'Video grid'],
  },
  'library-event-popup': {
    ...
    uses: ['Popup', 'Content badge', 'Progress indicator', 'Loading', 'Play button'],
  },
  'library-event-popup-live': {
    ...
    uses: ['Popup', 'Content badge', 'Progress indicator', 'Play button', 'Video grid'],
  },
  'library-gis-monitor': {
    title: 'GIS 관제',
    ...
    uses: ['Top navigation', 'Tab', 'Alert', 'Content badge', 'Tree/List', 'Map overlay'],
  },
  'library-settings': {
    title: '장비 관리',
    ...
    uses: ['Top navigation', 'Tab', 'Table', 'Toolbar', 'Tree/List'],
  },
  'library-data-retention': {
    title: '데이터 보관기간 설정',
    description: 'PREVAX 4 설정 > 데이터 보관기간 설정 화면을 디자인 시스템 토큰으로 재현한 예시입니다. 설정(장비 관리)과 동일한 타이틀바·탭·설정 네비 크롬을 공유하며(데이터 보관기간 설정 선택 상태), 상단 [적용] 버튼과 분석 유형별 보관기간 카드 그리드(이벤트·교통량·대기행렬·속도·점유율·통행량·진출입)로 구성됩니다. 각 카드는 라벨 + 값 입력 박스 + 단위(분·일·개월·년) 행으로 이루어지며, 이벤트 카드는 실시간 이벤트(드롭다운) 등 추가 항목을 포함합니다. 값은 예시값이며, 색·타이포·간격은 토큰(T/TYPE/W/SP)만 사용해 Library 설정 화면들과 시각 일관성을 유지합니다.',
    uses: ['Top navigation', 'Tab', 'Tree/List', 'Text field', 'Select', 'Button'],
  },
  'library-events': {
    title: '이벤트 관리',
    ...
    uses: ['Top navigation', 'Tab', 'Table', 'Switch', 'Search field', 'Toolbar'],
  },
  'library-alarm-settings': {
    ...
    uses: ['Top navigation', 'Tab', 'Alert', 'Card', 'Checkbox', 'Select', 'Text field', 'Button', 'Popup', 'Chip', 'List cell', 'Content badge'],
  },
  'library-history': {
    title: '이력 조회',
    ...
    uses: ['Top navigation', 'Tab', 'Table', 'Filter button', 'Content badge', 'Toolbar', 'Filter bar', 'Property grid'],
  },
  'library-stats': {
    title: '통계 보고서',
    ...
    uses: ['Top navigation', 'Tab', 'Radio', 'Checkbox', 'Date picker', 'Button', 'Icon button', 'Page counter', 'Toolbar', 'Tree/List'],
  },
  'library-ux-agent': {
    title: 'UX Agent 활용성 리뷰',
    description: '헤르메스 에이전트 기반 UX 자동화 실증 분석 보고서 (3장). 추진 배경·시장 현황·아키텍처 분석·PREVAX 4 실증 사례·핵심 인사이트·제언으로 구성됩니다.',
    uses: ['Table', 'Section message', 'Grid layout'],
  },
  'library-event-search': {
    ...
    uses: ['Top navigation', 'Tab', 'Date picker', 'Checkbox', 'Button', 'Segmented control', 'Pagination', 'Content badge', 'Text field', 'Tree/List', 'Thumbnail grid'],
  },
  'library-camera-form': {
    title: '카메라 정보 관리',
    ...
    uses: ['Popup', 'Text field', 'Select', 'Button'],
  },
  'library-camera-form2': {
    ...
    uses: ['Popup', 'Tab', 'Text field', 'Select', 'Button'],
  },
  'library-event-def-add': {
    ...
    uses: ['Popup', 'Select', 'Text field', 'Button', 'Checkbox', 'Icon button'],
  },
  'library-camera-group': {
    ...
    uses: ['Top navigation', 'Tab', 'Text field', 'Button', 'Checkbox', 'Search field', 'Tooltip', 'Content badge', 'List cell', 'Tree/List'],
  },
  'library-event-activation': {
    ...
    uses: ['Top navigation', 'Table', 'Tree/List', 'Checkbox', 'Text field', 'Button', 'Search field', 'Switch', 'Chip', 'Tooltip', 'Content badge', 'Fallback view'],
  },
  'library-permission2': {
    ...
    uses: ['Table', 'Checkbox', 'Button', 'Content badge', 'Tree/List'],
  },
  'library-permission': {
    title: '권한 설정',
    ...
    uses: ['Top navigation', 'Tab', 'Checkbox', 'Table', 'Button', 'Tree/List'],
  },
};
